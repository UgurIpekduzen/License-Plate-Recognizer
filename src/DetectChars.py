...
def generateData(path):
    ...
    for dirName in os.listdir(path):
        ...
        strSubDirPath = os.path.join(path, dirName)
        ...
        for fileName in os.listdir(strSubDirPath):
            ...
            for npaContour in npaContours:  # for each contour
                if cv2.contourArea(npaContour) > MIN_CONTOUR_AREA:  # if contour is big enough to consider
                    ...
                    cv2.imshow("charset", imgTrainingChar)
                    intClassifications.append(ord(dirName))  # append classification char to integer list of chars (we will convert to float later before writing to file)
