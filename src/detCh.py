...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        possiblePlate.imgGrayscale, possiblePlate.imgThresh = EditFrame.preprocess(possiblePlate.imgPlate)     # preprocess to get grayscale and threshold images
...
###################################################################################################
def findListOfListsOfMatchingChars(listPossibleChars):
            # with this function, we start off with all the possible chars in one big list
            # the purpose of this function is to re-arrange the one big list of chars into a list of lists of matching chars,
            # note that chars that are not found to be in a group of matches do not need to be considered further
    listOfListsOfMatchingChars = []                  # this will be the return value

    for possibleChar in listPossibleChars:                        # for each possible char in the one big list of chars
        listMatchingChars = findListOfMatchingChars(possibleChar, listPossibleChars)        # find all chars in the big list that match the current char

        listMatchingChars.append(possibleChar)                # also add the current char to current possible list of matching chars

        if len(listMatchingChars) < MIN_NUMBER_OF_MATCHING_CHARS:     # if current possible list of matching chars is not long enough to constitute a possible plate
        ...
        listOfListsOfMatchingChars.append(listMatchingChars)      # so add to our list of lists of matching chars

        listOfPossibleCharsWithCurrentMatchesRemoved = []

                                                # remove the current list of matching chars from the big list so we don't use those same chars twice,
                                                # make sure to make a new big list for this since we don't want to change the original big list
        listPossibleCharsWithCurrentMatchesRemoved = list(set(listPossibleChars) - set(listMatchingChars))

        recursiveListOfListsOfMatchingChars = findListOfListsOfMatchingChars(listPossibleCharsWithCurrentMatchesRemoved)      # recursive call
...
def findListOfMatchingChars(possibleChar, listOfChars):
    ...
    listMatchingChars = []                # this will be the return value
    ...
    for possibleMatchingChar in listOfChars:                # for each char in big list
        ...
        if (distanceBetweenChars < (possibleChar.fltDiagonalSize * MAX_DIAG_SIZE_MULTIPLE_AWAY) and
            ...
            listMatchingChars.append(possibleMatchingChar)        # if the chars are a match, add the current char to list of matching chars
        # end if
    # end for

    return listMatchingChars                  # return result
# end function

# use Pythagorean theorem to calculate distance between two chars
def distanceBetweenChars(firstChar, secondChar):
    intX = abs(firstChar.intCenterX - secondChar.centerX)
    intY = abs(firstChar.intCenterY - secondChar.centerY)
...
def angleBetweenChars(firstChar, secondChar):
    ...
    if fltAdj != 0.0:                           # check to make sure we do not divide by zero if the center X positions are equal, float division by zero will cause a crash in Python
        angleRad = math.atan(fltOpp / fltAdj)      # if adjacent is not zero, calculate angle
    else:
        angleRad = 1.5708                          # if adjacent is zero, use this as the angle, this is to be consistent with the C++ version of this program
    # end if

    angleDeg = angleRad * (180.0 / math.pi)       #Dereceye çevir
