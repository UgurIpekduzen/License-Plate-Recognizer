...
def detectPlatesInScene(imgOriginalScene):
    ...
    imgGrayscaleScene, imgThreshScene = DetectChars.EditImage.preprocess(imgOriginalScene)         # preprocess to get grayscale and threshold images
    ...
    if Constant.showSteps == True: # show steps #######################################################
        ...
        cv2.drawContours(imgContours, contours, -1, Constant.SCALAR_WHITE)
    ...
    if Constant.showSteps == True: # show steps #######################################################
        ...
        for i in range(0, len(listOfPossiblePlates)):
            ...
            cv2.line(imgContours, tuple(p2fRectPoints[0]), tuple(p2fRectPoints[1]), Constant.SCALAR_RED, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[1]), tuple(p2fRectPoints[2]), Constant.SCALAR_RED, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[2]), tuple(p2fRectPoints[3]), Constant.SCALAR_RED, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[3]), tuple(p2fRectPoints[0]), Constant.SCALAR_RED, 2)
...
def findPossibleCharsInScene(imgThresh):
    ...
    for i in range(0, len(contours)):                       # for each contour
        ...
        if Constant.showSteps == True: # show steps ###################################################
            cv2.drawContours(imgContours, contours, i, Constant.SCALAR_WHITE)
        # end if # show steps #####################################################################

        possibleChar = DetectChars.PossibleChar.PossibleChar(contours[i])
...
def extractPlate(imgOriginal, listOfMatchingChars):
    ...
    intPlateWidth = int((listOfMatchingChars[len(listOfMatchingChars) - 1].intBoundingRectX + listOfMatchingChars[len(listOfMatchingChars) - 1].intBoundingRectWidth - listOfMatchingChars[0].intBoundingRectX) * Constant.PLATE_WIDTH_PADDING_FACTOR)
    ...
    intPlateHeight = int(fltAverageCharHeight * Constant.PLATE_HEIGHT_PADDING_FACTOR)
